...
public class AstIdentifier extends AstNode implements IdentifierNode {
	...
	public Class<?> getType(Bindings bindings, ELContext context) {
		...
		if (!context.isPropertyResolved()) {
			throw new PropertyNotFoundException(org.activiti.engine.impl.juel.LocalMessages.get("error.identifier.property.notfound", name));
		}
		return result;
	}
	...
	protected Method getMethod(Bindings bindings, ELContext context, Class<?> returnType, Class<?>[] paramTypes) {
		...
		if (value == null) {
			throw new MethodNotFoundException(org.activiti.engine.impl.juel.LocalMessages.get("error.identifier.method.notfound", name));
		}
		...
		throw new MethodNotFoundException(org.activiti.engine.impl.juel.LocalMessages.get("error.identifier.method.notamethod", name, value.getClass()));
	}
	...
	public Object invoke(Bindings bindings, ELContext context, Class<?> returnType, Class<?>[] paramTypes, Object[] params) {
		...
		try {
			...
			throw new ELException(org.activiti.engine.impl.juel.LocalMessages.get("error.identifier.method.access", name));
		} catch (IllegalArgumentException e) {
			throw new ELException(org.activiti.engine.impl.juel.LocalMessages.get("error.identifier.method.invocation", name, e));
