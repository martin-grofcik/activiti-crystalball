...
import org.activiti.crystalball.processengine.wrapper.activiti.HistoryServiceWrapperImpl;
...
public class AuditTrailProcessDiagramGeneratorTest extends PluggableActivitiTestCase {
	...
	public void test() throws IOException {
		...
		generator.setHistoryService(new HistoryServiceWrapperImpl( processEngine.getHistoryService()));
		generator.setRepositoryService(new RepositoryServiceWrapperImpl((RepositoryServiceImpl)processEngine.getRepositoryService()));
		...
		while (processEngine.getTaskService().createTaskQuery().count() >0 ) {
			...
			ImageIO.write( ImageIO.read(new ByteArrayInputStream( generator.generateLayer( "png", params)))
					, "png"
					, generatedFile);
			assertTrue(FileUtils.contentEquals(generatedFile, new File("src/org.activiti.crystalball.processengine.wrapper.test/resources/org/activiti/crystalball/diagram/auditTrail"+i+".png")));
		}
	}
